...
from mphot import generate_system_response, get_precision
...
def get_models(
    ...
    for filter_path in filter_paths:
        name, system_response = generate_system_response(instrument_efficiency_path, filter_path)
        response = {
            "name" : name,
        ...
        for bin in binning:
            for setting in gain_setting:
                ...
                for target_name, Teff, dist, props_s, m_exp in zip(target_names, target_Teffs, target_dists, props_sky, max_exp):
                    ...
                    precision = get_precision(props_i, props_s, Teff, dist, extended=True)
                    ...
                    if precision_curve:
                        for i in N_stars:
                            image_precision_i, binned_precision_i, components_i = get_precision(props_i, props_s, Teff, dist, exp_time=exp_time, N_star=i, extended=True)
